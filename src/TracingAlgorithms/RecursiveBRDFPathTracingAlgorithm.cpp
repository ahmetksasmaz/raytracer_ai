    ...
    int current_recursion, std::shared_ptr<PathTracerSettings>& path_tracer_settings)
{
    ...
    // #####################################################################
    // UNTIL HERE, EVERYTING IS SAME AS RAY TRACING WITH DIRECT ILLUMINATION
    ...
    if(current_recursion < path_tracer_settings->max_recursion_depth){
        ...
        for(int i = 0; i < sample_count; i++){
            ...
            Vec3f tangent = normalize(cross(normal_prime, hit_normal));
            Vec3f bitangent = cross(hit_normal, tangent);
            
            Vec3f sample_direction = normalize(tangent * (std::sin(theta) * std::cos(phi)) +
            bitangent * (std::sin(theta) * std::sin(phi)) +
            hit_normal * std::cos(theta));
            Ray sample_ray = {
                ray.pixel_,
                intersection_point,
                sample_direction,
                ray.diff_,
                ray.time_};
            Vec3f sample_color = RecursiveBRDFPathTracingAlgorithm(
            ...
            if(path_tracer_settings->mis_balance_enabled){
                ...
                Vec3f light_hit_tangent_vector;
                Vec3f light_hit_bitangent_vector;
                Vec2f light_hit_u_vector;
                Vec2f light_hit_v_vector;
                std::shared_ptr<BoundingVolumeHierarchyElement> light_hit_object_ptr = nullptr;
                for (auto& light_object : light_objects_)
                {
                    FP_PRECISION temp_hit = std::numeric_limits<FP_PRECISION>::max();
                    Vec3f normal;
                    std::shared_ptr<BaseObject> hit_object_casted =
                        std::dynamic_pointer_cast<BaseObject>(light_object);
                    if (light_object->Intersect(sample_ray, temp_hit, normal, light_hit_tex_coords, light_hit_u_vector, light_hit_v_vector, light_hit_tangent_vector, light_hit_bitangent_vector, false))
                    {
                        if (light_hit > temp_hit)
                        {
                            light_hit = temp_hit;
                            light_hit_object_ptr = light_object;
                            light_hit_normal = normal;
                        }
                    }
                }
                if(light_hit_object_ptr && !ShadowCheck(sample_direction, light_hit)){
                    // CALCULATE WEIGHT
                    FP_PRECISION light_pdf = 0.0;
                    auto light_object_casted = std::dynamic_pointer_cast<ObjectLightSource>(light_hit_object_ptr);
                    Vec3f sample_point;
                    Vec3f sample_normal;
