...
PlyFile *ply_read(FILE *fp, int *nelems, char ***elem_names)
{
  ...
  if (!words || !equal_strings (words[0], const_cast<char*>("ply")))
    return (NULL);
  ...
  while (words) {
    ...
    if (equal_strings (words[0], const_cast<char*>("format"))) {
      if (nwords != 3)
        return (NULL);
      if (equal_strings (words[1], const_cast<char*>("ascii")))
        plyfile->file_type = PLY_ASCII;
      else if (equal_strings (words[1], const_cast<char*>("binary_big_endian")))
        plyfile->file_type = PLY_BINARY_BE;
      else if (equal_strings (words[1], const_cast<char*>("binary_little_endian")))
    ...
    else if (equal_strings (words[0], const_cast<char*>("element")))
      add_element (plyfile, words, nwords);
    else if (equal_strings (words[0], const_cast<char*>("property")))
      add_property (plyfile, words, nwords);
    else if (equal_strings (words[0], const_cast<char*>("comment")))
      add_comment (plyfile, orig_line);
    else if (equal_strings (words[0], const_cast<char*>("obj_info")))
      add_obj_info (plyfile, orig_line);
    else if (equal_strings (words[0], const_cast<char*>("end_header")))
      break;
...
int get_prop_type(char *type_name)
{
  ...
  for (i = PLY_START_TYPE + 1; i < PLY_END_TYPE; i++)
    if (equal_strings (type_name, const_cast<char*>(type_names[i])))
      return (i);
...
void add_property (PlyFile *plyfile, char **words, int nwords)
{
  ...
  if (equal_strings (words[1], const_cast<char*>("list"))) {       /* is a list */
