...
// loadFromXml lived here. It was removed with the pipeline split: it had
// drifted far behind the JSON loader (no Plane, MeshInstance, LightMesh,
// LightSphere, BRDFs, DirectionalLight, SpotLight,
// SphericalDirectionalLight, Tonemap or Renderer), no XML scene exists in
// the repo, and .gitignore excludes *.xml so none could be committed. A
// loader that silently renders a different scene than the file describes is
// worse than no loader, so Scene::LoadScene now rejects .xml outright.
...
void parser::RawScene::loadFromJSON(const std::string &filepath) {
  ...
  if (json_data.contains("Cameras")) {
    ...
    for (const auto& cam : json_cams) {
      ...
      // A "Sensor" block used to be read here. Sensors are configured in
      // their own files now (include/Sensor/SensorConfig.hpp); warn rather than
      // ignore, so a scene carrying the old block does not silently render
      // without the sensor its author expected.
      if (cam.contains("Sensor")) {
        std::cerr << "Warning: '" << filepath
                  << "' declares a camera \"Sensor\" block, which is no longer"
                     " read. Sensor simulation is a separate program now: move"
                     " the block into its own config file and pass it to the"
                     " sensor_* stages. See tools/pipeline.sh."
                  << std::endl;
      }
