...
class TorranceSparrow : public BaseBRDF {
 public:
    ...
    Vec3f Evaluate(const Vec3f &incoming_direction, const Vec3f &outgoing_direction, const Vec3f &normal, const Vec3f &kd, const Vec3f &ks, FP_PRECISION refraction_index, FP_PRECISION absorption_index) const override {
        ...
        FP_PRECISION g_value = std::min((FP_PRECISION)1.0, std::min((FP_PRECISION)(2.0 * dot(normal, half_vector) * dot(normal, outgoing_direction) / dot(outgoing_direction, half_vector)),
                                                (FP_PRECISION)(2.0 * dot(normal, half_vector) * dot(normal, incoming_direction) / dot(outgoing_direction, half_vector))));
        ...
        FP_PRECISION f_theta_value = f_zero_value + (1.0 - f_zero_value) * std::pow((1.0 - dot(half_vector, incoming_direction)), 5.0);
