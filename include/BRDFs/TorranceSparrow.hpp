...
class TorranceSparrow : public BaseBRDF {
 public:
    ...
    Vec3f Evaluate(const Vec3f &incoming_direction, const Vec3f &outgoing_direction, const Vec3f &normal, const Vec3f &kd, const Vec3f &ks, FP_PRECISION refractive_index) const override {
        ...
        FP_PRECISION r0_value = std::pow(refractive_index - 1.0, 2) / std::pow(refractive_index + 1.0, 2);
        FP_PRECISION f_value = r0_value + (1.0 - r0_value) * std::pow((1.0 - dot(half_vector, outgoing_direction)), 5.0);
        FP_PRECISION specular_term = (d_alpha * g_value * f_value) / (4.0 * dot(incoming_direction, normal) * dot(outgoing_direction, normal));
        if(kd_fresnel_){
            return kd * (1.0 - f_value) * (1.0 / M_PI) + ks * specular_term;
        }
        else{
