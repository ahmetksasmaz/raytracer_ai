...
dataset/    100 procedural Cornell boxes; scenes + metadata committed, renders not
...
`dataset/` is 100 procedurally generated Cornell boxes — 90 diffuse (N spheres [0,20], M cubes [0,20], K illuminants [1,6]) and 10 with glass and mirror objects (N [0,10], M [1,20], K [1,6]). Every scene ships its full spectral decomposition, which is the reason to build a dataset with this renderer rather than another: `_illumination.exr` is per-pixel ground truth for the illuminant.

```bash
python3 dataset/generate_dataset.py            # 100 scenes + metadata, instant
./dataset/render_all.sh                        # ~7 h, ~10 GB, resumable
./dataset/render_all.sh --scenes 5             # check before committing the hours
python3 dataset/verify_dataset.py              # structural + render sanity
```

512×512 at 512 spp, measured at ~4 min/scene. **`render_all.sh` is resumable** — a scene whose `_radiance.exr` exists is skipped — and prints a per-scene ETA from the running mean, because a seven-hour job will be interrupted and a bare spinner is useless over that duration. It tees to `dataset/render.log`.

Scene construction is built on `hyperspectral_box.json`'s open 5-wall box with the camera outside, not `cornell_brute.json`'s closed one. Placement rejection-samples against two conditions — inside the box by the object's own radius, and projecting wholly inside the frame via the camera's actual projection. **"Visible" means in frame, not unoccluded**: overlap is deliberately allowed, so an object can still be hidden behind another. About 45% of objects rest on the floor; entirely floating scenes look wrong.

**The decomposition is exact per sample, not per pixel.** `radiance = reflectance ⊙ illumination` holds for every sample, but the 3×3 reconstruction filter mixes surfaces within a pixel at a geometric silhouette, and `mean(R⊙E) != mean(R)⊙mean(E)` there. On the ColorChecker chart this is invisible (2.7e-08) because the illumination is constant across the frame; on a box with 40 objects it is ~4% relative RMSE at 256², falling as √(pixel count) as the edge fraction shrinks. That is a property of filtered AOVs, not a bug — but a consumer of the dataset comparing the three cubes pixel-wise needs to know it.

`verify_dataset.py` checks metadata counts against the JSON, object containment and framing, that the specular scenes really contain dielectrics and mirrors at `MaxRecursionDepth >= 12`, and that no render came out effectively black. Each check was confirmed to fail on a deliberately broken copy — the black-render check is the important one, since a scene whose emitters are wound backwards produces a plausible dark image rather than an error.
