...
demo/       ColorChecker under 5 illuminants through all 30 cameras; outputs gitignored
...
`demo/` is the worked example: one ColorChecker chart under 5 illuminants (`d65`, `incandescent`, `fl11`, `led_b3`, `hps`), each render developed through **all 30 measured cameras** — 150 pairs, 9 images each.

```bash
python3 demo/generate_demo.py     # 5 scenes + 30 sensor configs
./demo/run_demo.sh                # ~5 min, ~750 MB, all gitignored
./demo/run_demo.sh --sensors 3    # a quick look
```

Outputs land in `demo/rendered_<illuminant>/` and `demo/<sensor>_<illuminant>/`. Only the scripts, scenes and sensor configs are committed; the results regenerate.
